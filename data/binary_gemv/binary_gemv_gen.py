...
for y in range(0, DIM_OUT):
    for x in range(0, DIM_IN):
        ...
        popcount = bin(xnor_result).count('1')
...
np.save("bin_gemv_input_" + str(DIM_OUT) + "x" + str(DIM_IN), batch_in)
np.save("bin_gemv_weight_" + str(DIM_OUT) + "x" + str(DIM_IN), data_w)
np.save("bin_gemv_output_" + str(DIM_OUT) + "x" + str(DIM_IN), batch_out)
np.save("bin_gemv_test_output_" + str(DIM_OUT) + "x" + str(DIM_IN), batch_out2)
print(batch_in)
print(batch_out)
print(batch_out2)
